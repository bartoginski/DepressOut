<template>
  <div class="container flex-row flex md:items-center gap-0 md:gap-40">
    <div class="flex-1 justify-end hidden md:block">
      <img src="~/assets/img/help_log.svg" alt="">
    </div>
    <div class="text-white text-center md:text-left flex-1 justify-center md:justify-end flex-col flex md:items-center gap-10 md:gap-0">
      <h2 class="text-6xl">Jesteśmy tu, żeby pomóc</h2>
      <p class="text-xl">
        Tworzymy projekt mający na celu pomóc ludziom dotkniętym depresją. 
        Dlatego stworzyliśmy ankietę by lepiej zrozumieć problem. 
        Dziękujemy za poświęcony czas.
      </p>
    </div>
  </div>
